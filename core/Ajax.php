...
class Ajax {
	...
	public function register() {
		...
		add_action(
			'wp_ajax_nopriv_get_remaining_attempts_message',
			array(
				$this,
	...
	public function app_acl_add_rule_callback() {
		...
		if ( ! empty( $_POST['pattern'] ) && ! empty( $_POST['rule'] ) && ! empty( $_POST['type'] ) ) {
			...
			if ( $response = LimitLoginAttempts::$cloud_app->acl_create(
				array(
					'pattern' => $pattern,
					'rule'    => $rule,
	...
	public function app_acl_remove_rule_callback() {
		...
		if ( ! empty( $_POST['pattern'] ) && ! empty( $_POST['type'] ) ) {
			...
			if ( $response = LimitLoginAttempts::$cloud_app->acl_delete(
				array(
					'pattern' => $pattern,
					'type'    => ( $type === 'ip' ) ? 'ip' : 'login',
				)
	...
	public function app_load_successful_login_callback() {
		...
		if ( $custom === 'true' ) {
			...
			$local_data     = array(
				...
				'login'    => 'admin',
				'ip'       => '11.22.33.44',
				'location' => array( 'country_code' => 'US' ),
					...
					<tr>
						...
						<td class="cell-role <?php echo $limited ? 'llar-blur-block-cell' : ''; ?>">
							<?php if ( isset( $item['roles'] ) && is_array( $item['roles'] ) ) : ?>
								<span class="hint_tooltip-parent">
									<?php
									$admin_key = array_search( 'administrator', $item['roles'] );
									if ( $admin_key !== false ) :
										?>
										<span><?php echo esc_html( $item['roles'][ $admin_key ] ); ?></span>
										<?php
										unset( $item['roles'][ $admin_key ] );
					...
					<?php if ( $limited ) : ?>
						...
						<?php else : ?>
							<td colspan="2" style="max-width: 200px">
								...
								<?php
								if ( ! empty( $item['location']['timezone_name'] ) ) :
								...
								<?php
								if ( ! empty( $item['location']['isp_name'] ) && ! empty( $item['location']['organization_name'] ) ) :
	...
	public function app_load_acl_rules_callback() {
		...
		$acl_list = LimitLoginAttempts::$cloud_app->acl(
			array(
				'type'   => $type,
				'limit'  => $limit,
				'offset' => $offset,
	...
	public function app_toggle_country_callback() {
		...
		if ( $action_type === 'add' ) {
			...
			$result = LimitLoginAttempts::$cloud_app->country_add(
				array(
					'code' => $code,
				)
			);
			...
			$result = LimitLoginAttempts::$cloud_app->country_remove(
				array(
					'code' => $code,
				)
			);
	...
	public function app_country_rule_callback() {
		...
		$result = LimitLoginAttempts::$cloud_app->country_rule(
			array(
				'rule' => $rule,
			)
		);
	...
	public function subscribe_email_callback() {
		...
		if ( ! empty( $email ) && is_email( $email ) ) {
			...
			if ( $is_subscribe_yes ) {
				$response = Http::post(
					'https://api.limitloginattempts.com/my/key',
					array(
						'data' => array(
							'email' => $email,
						),
			...
			wp_send_json_success(
				array(
					'email'            => $email,
					'is_subscribe_yes' => $is_subscribe_yes,
				)
			);
	...
	/**
	 * Frontend login-page cache probe: delegates to LoginPageCacheDetector.
	 */
	public function login_cache_check_callback() {
		LimitLoginAttempts::$instance->get_login_page_cache_detector()->handle_frontend_check();
	}

	/**
	 * Dismiss the login-page cache admin notice (admins only).
	 */
